...
from fedml_api.standalone.superfednas.elastic_nn.ofa_resnets_32x32 import (
    OFAResNets32x32_50l,
    ResNets32x32,
)
...
class ServerResnet(BaseServerModel):
    def __init__(
        self,
        init_params,
        sampling_method,
        ...
        if bn_gamma_zero_init:
            self._initialize_gamma()

    def init_model(self, init_params):
        return OFAResNets32x32_50l(**init_params)

    def _initialize_gamma(self):
        for m in self.model.modules():
            if isinstance(m, DynamicResNetBottleneckBlock):
                nn.init.constant_(m.conv3.bn.bn.weight, 0)
    ...
    def active_subnet_index(self):
        ...
        if self.model.input_stem_skipping <= 0:
            mapping_active_subnet_to_supernet["input_stem"][1] = 1
            mapping_active_subnet_to_supernet["stem_channels"][1] = (
                self.model.input_stem[1].conv.active_out_channel,
                self.model.input_stem[0].active_out_channel,
            )
            # Note that identity layer has channel dimensions: Identity(self.model.input_stem[0].active_out_channel, self.model.input_stem[0].active_out_channel)
            mapping_active_subnet_to_supernet["input_stem"][2] = 2
            mapping_active_subnet_to_supernet["stem_channels"][2] = (
                self.model.input_stem[2].active_out_channel,
                self.model.input_stem[1].conv.active_out_channel,
            )
        else:
            mapping_active_subnet_to_supernet["input_stem"][1] = 2
            mapping_active_subnet_to_supernet["stem_channels"][1] = (
                self.model.input_stem[2].active_out_channel,
                self.model.input_stem[0].active_out_channel,
            )
        input_channel = self.model.input_stem[2].active_out_channel
        ...
        for key in local_model_params:
            ...
            if "conv.weight" in key:
                ...
                if layer_type == "input_stem":
                    """
                    Input Stem: up to 3 Dynamic Convolutional Layers (depends on depth of input stem)
                        First Conv Layer (takes input RGB image): 3 -> active_middle_channels
                        Second Conv Layer : active_middle_channels -> active_middle_channels
                        Third Conv Layer : active_middle_channels -> active_out_channels
                        For any given layer, number of input channels = local_model_index["stem_channels"][ind][1]
                        and number of output channels = local_model_index["stem_channels"][ind][0]
                    """
                    ...
                    ResNet Bottleneck Block Structure
                        3 convolutional layers followed by a summationn with downsample residual
                        Conv 1: in_channels (local_model_index["channels"][ind][2]) -> active_middle_channels (local_model_index["channels"][ind][1])
                        Conv 2: active_middle_channels -> active_middle_channels
                        Conv 3: active_middle_channels -> active_out_channels (local_model_index["channels"][ind][0])
                        Downsample: in_channels -> active_out_channels
                    """
                    if "conv1" in key:
    ...
    def max_subnet_arch(self):
        ...
        return {"d": [2, 2, 2, 2, 2], "e": expand_setting}
    ...
    def min_subnet_arch(self):
        ...
        return {"d": [0, 0, 0, 0, 0], "e": expand_setting}
    ...
    def random_subnet_arch(self):
        ...
        # sample depth
        depth_setting = [np.random.choice([max(depth_list), min(depth_list)])]
    ...
    def mutate_sample(self, sample_arch, mut_prob):
        ...
        # Resample depth
        for i in range(len(ResNets32x32.BASE_DEPTH_LIST)):
            if random.random() < mut_prob:
