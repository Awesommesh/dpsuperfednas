...
from fedml_api.standalone.superfednas.elastic_nn.TCN.elastic_char_tcnn import (
    ElasticCharTCN
)
...
class ServerElasticCharTCNN(BaseServerModel):
    def __init__(
        self,
        init_params,
        sampling_method,
        ...
        super(ServerElasticCharTCNN, self).__init__(
            init_params,
            sampling_method,
            num_cli_total,
    ...
    def init_model(self, init_params):
        return ElasticCharTCN(**init_params)
            ...
            if "tcn" in key:
                ...
                if "weight_g" in key:
                    if conv_id == "conv1":
                        ...
                        raise Exception("Unexpected weight_g parameter found: " + key)
                    ...
                    if conv_id == "conv1":
                        ...
                        raise Exception("Unexpected weight_v parameter found: " + key)
                    ...
                    if "bias" not in key:
                        big_key += "weight"
                        shared_param_sum[big_key][:out, :in_size, :] += weight * local_model_params[key]
                        shared_param_count[big_key][:out, :in_size, :] += weight
                    else:
                        big_key += "bias"
                        shared_param_sum[big_key][:out] += weight * local_model_params[key]
                        shared_param_count[big_key][:out] += weight
                elif "bias" in key:
                    if conv_id == "conv1":
                        shared_param_sum[key][:middle] += weight*local_model_params[key]
                        shared_param_count[key][:middle] += weight
                        ...
                        raise Exception("Unexpected bias parameter found: " + key)

                else:
                    raise Exception("Unexpected parameter found: " + key)
            else:
