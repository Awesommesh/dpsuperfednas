...
class FedOptAggregator(object):
    ...
    def get_model_params(self):
        ...
        return self.trainer.client_model.parameters()
    ...
    def set_model_global_grads(self, new_state):
        new_model = copy.deepcopy(self.trainer.client_model)
        new_model.load_state_dict(new_state)
        with torch.no_grad():
            for parameter, new_parameter in zip(
                    self.trainer.client_model.parameters(), new_model.parameters()
            ):
                parameter.grad = parameter.data - new_parameter.data
                # because we go to the opposite direction of the gradient
        model_state_dict = self.trainer.client_model.state_dict()
        new_model_state_dict = new_model.state_dict()
        for k in dict(self.trainer.client_model.named_parameters()).keys():
